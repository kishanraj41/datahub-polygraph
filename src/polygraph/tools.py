"""Polygraph's query surface, as plain functions.

These are the single implementation behind both the MCP server
(``polygraph.mcp_server``) and the ``polygraph ask`` CLI. Keeping one copy is
not tidiness: two implementations of "can I trust this asset" would drift, and
the whole project is about catching claims that drifted from reality.

Every function returns ``evidence_available``. A caller that cannot find
evidence must say so rather than returning a plausible-looking empty result --
an empty answer reads as "nothing wrong", which is the failure mode Polygraph
